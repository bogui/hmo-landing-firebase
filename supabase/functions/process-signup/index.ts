...
Deno.serve(async (req) => {
  ...
  try {
    const { action, recaptchaToken, payload } =
      (await req.json()) as ProcessSignupRequest;
    if (action !== "signup" || !recaptchaToken || !payload) {
      return new Response(
        JSON.stringify({ status: "server_error", message: "Bad request" }),
        {
          status: 400,
    ...
    if (!recaptcha.ok) {
      return new Response(
        JSON.stringify({ status: "recaptcha_failed" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        },
