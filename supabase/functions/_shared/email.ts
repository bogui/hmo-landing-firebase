  ...
  return `
      ...
      <head>
        ...
        <style>
          ...
          .container { background-color: white; border-radius: 12px; padding: 30px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); }
          ...
          .field { margin-bottom: 15px; padding: 10px; background-color: #f8f9fa; border-radius: 8px; border-left: 4px solid #3b82f6; }
      ...
      <body>
        <div class="container">
          ...
          <div class="field"><div class="field-label">👤 Име:</div><div class="field-value">${data.firstName}</div></div>
          <div class="field"><div class="field-label">👤 Фамилия:</div><div class="field-value">${data.lastName}</div></div>
          <div class="field"><div class="field-label">🏢 Име на компанията:</div><div class="field-value">${data.company}</div></div>
          <div class="field"><div class="field-label">📧 Email:</div><div class="field-value"><a href="mailto:${data.email}" style="color: #3b82f6;">${data.email}</a></div></div>
          <div class="field"><div class="field-label">📞 Телефон:</div><div class="field-value"><a href="tel:${data.phone}" style="color: #3b82f6;">${data.phone}</a></div></div>
          <div class="footer">
  ...
  return `
      ...
      <head>
        ...
        <style>
          ...
          .next-steps { background-color: #fef3c7; padding: 20px; border-radius: 8px; border-left: 4px solid #f59e0b; margin: 20px 0; }
          .footer { margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center; color: #6b7280; font-size: 14px; }
...
export async function sendAdminNotification(
  ...
  const messageData = {
    ...
    subject: alreadyRegistered
      ? "Заявка от вече регистриран клиент"
      : "Заявка от нов клиент",
    text: getNotificationTextTemplate(data),
    html: getNotificationHtmlTemplate(data),
  };
...
export async function sendConfirmationEmail(data: EmailData, to: string) {
  const messageData = {
    from: "Hyper-M <info@hyper-m.online>",
    to: [to],
    subject: "Потвърждение за регистрация - Hyper-M",
